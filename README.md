...
modules/
├── shell/            #   common.nix (starship/zoxide/fzf), zsh.nix (both), bash.nix (Linux)
├── linux-desktop.nix #   Linux-only GUI apps + GNOME-adjacent packages
├── gnome.nix          #   dconf settings (generated by gnome-settings-export)
└── programs/          #   git, vscode, zen (Linux-only unless noted)
config/                # raw config files home-manager symlinks into $HOME
├── wezterm/           #   .wezterm.lua
└── tmux/              #   .tmux.conf
bin/                   # helper scripts (ts-ssh.sh, gnome-settings-export)
...
Files like `.wezterm.lua` and `.tmux.conf` are symlinked back to this repo via
`mkOutOfStoreSymlink`, so edits to them take effect immediately without a
rebuild.
